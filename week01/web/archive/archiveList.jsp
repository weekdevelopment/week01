...
<%
    ...
    //4. 받아온 결과셋(ResultSet) 을 질문및답변 목록(archList)에 불러와 하나의 레코드씩 담기
    List<Archive> archList = new ArrayList<>();
    while (rs.next()) {
        Archive arch = new Archive();
        arch.setNo(rs.getInt("ano"));
        arch.setTitle(rs.getString("title"));
        arch.setResdate(rs.getString("resdate"));
        arch.setAttach(rs.getString("attach"));
        archList.add(arch);
    }
