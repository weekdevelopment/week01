...
-usecase diagram 작성
-WBS 작성
-ERD, 클래스 다이어 그램 작성(언론보도 DB) 및 종합
[기능구현]
-언론보도 구현 : 제목, 작성일, 링크를 DB에 저장 후 불러오기
-기능 구현 영상 촬영 : 전체 홈페이지 기능 구현 동영상 촬영 및 업로드
-unit task flow 작성 : 언론보도
이소윤
[설계]
-요구사항 작성
-unit task flow 작성 : 공지사항, 회사소개, 인재 채용
-ERD, 클래스 다이어그램 작성 : 공지사항 DB, 인재 채용 DB
[기능]
-관리자 페이지 제작 : 모든 게시판 리스트 보기, 각 게시판별 글 삭제, 수정 기능
-윜소식, 공지사항 구현 : DB에서 제목을 받아와 리스트로 만들고, 제목 클릭 시 내용 보기 기능.
-회사 소개, 인재 채용 구현 : 입사 지원 form을 입력 받아 DB에 연결 및 관리자 페이지에 표시
이은영
[설계]
-요구사항 분석
-프로토 타입 제작 : 카카오 oven 이용 제작.
-홈페이지 메인 인덱스, 프론트엔드, css
[기능]
-자주하는 질문 구현 : DB에서 가져와 sliding 기능 구현
-readme 디자인, 종합, 작성
한선
[기능]
-1:1 문의(게시판) 구현 : 질문과 답글을 나누어 리스트화, 답글은 질문글에 lev값을 부여하여 연결
-게시판 댓글 기능 : 1:1문의 글 상세보기 페이지 내 댓글 등록 및 삭제 가능
-관리자 페이지 : 회원 탈퇴, 관리자의 회원 강퇴 기능 구현
-고객만족센터, 오시는 길 구현
